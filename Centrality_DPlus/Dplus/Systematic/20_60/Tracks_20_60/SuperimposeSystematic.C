 void SuperimposeSystematic(Int_t binmin, Int_t binmax, Double_t thrmin, Double_t thrmax) {
  ...
  leg->AddEntry(h[1],"0ITS","epl");
  leg->AddEntry(h[2],"3ITSkAny","epl");
  leg->AddEntry(h[3],"90CrRows","epl");
  leg->AddEntry(h[4],"Find","epl");
  leg->AddEntry(h[5],"ClsVsPt","epl");
 ...
 // cOut->SaveAs(Form("AzimCorrDistr_Dplus_Canvas_PtIntBins%dto%d_PoolInt_thr%1.1fto%1.1f_Superimp.root",binmin,binmax,thrmin,thrmax));
  ...
  leg->SetFillStyle(0);
  leg->SetBorderSize(0);
  leg->AddEntry(hR[0],"0ITS/Std","epl");
  leg->AddEntry(hR[1],"3ITSkAny/Std","epl");
  leg->AddEntry(hR[2],"90CrRows/Std","epl");
  leg->AddEntry(hR[3],"Find/Std","epl");
  leg->AddEntry(hR[4],"ClsVsPt/Std","epl");
  leg->Draw();  
