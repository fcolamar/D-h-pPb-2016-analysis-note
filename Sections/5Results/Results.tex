\begin{comment}
Figures \ref{fig:D0corr2D}, ~\ref{fig:Dpluscorr2D} and ~\ref{fig:DStarcorr2D} show the fully corrected correlation 2D plots in $\Delta \eta -\Delta \phi$ for $\text{D}^0$, $\text{D}^+$  and $\text{D}^*+$ respectively, evaluated in the p-Pb data sample used in the analysis. Results have been extracted in four $\text{p}_T$ regions of the trigger particle: $3< p_{T}^{\text{D}^+}< 5 GeV/c$, $5< p_{T}^{\text{D}^+}< 8 GeV/c$, $8< p_{T}^{\text{D}^0}< 16 GeV/c$ and $16< p_{T}^{\text{D}^+}< 24 GeV/c$, with an associated track $\text{p}_T$ threshold varying in the following intervals: $ p_{T}^{\text{assoc}}> 0.3 GeV/c$, $0.3< p_{T}^{\text{assoc}}< 1 GeV/c$, $p_{T}^{\text{assoc}}> 1 GeV/c$, $1< p_{T}^{\text{assoc}}< 2 GeV/c$, $2< p_{T}^{\text{assoc}}< 3 GeV/c$ and $p_{T}^{\text{assoc}}> 2 GeV/c$.
...
\caption{Fully corrected distribution of $D^0$-hadrons correlations in $\Delta \eta -\Delta \phi$,  in different $\text{D}^0$ $\text{p}_T$ regions. Left: $3< p_{T}^{\text{D}^0}< 5 GeV/c$, Mid: $5< p_{T}^{\text{D}^0}< 8 GeV/c$ , High: $8< p_{T}^{\text{D}^0}< 16 GeV/c$  for associated $p_\text{T}$ threshold of 0.3 GeV/$c$ (upper row), $p_\text{T}$ threshold of 1 GeV/$c$ (middle row) and $p_\text{T}$ threshold of 2 GeV/$c$ (bottom row).}
\label{fig:D0corr2D}
\end{figure}
\begin{figure}[h]
...
%
%
Given the compatibility within the statistical and systematic uncertainties was found among the $\Dzero$, $\Dplus$ and $\Dstar$ azimuthal correlations, and since no difference is visible in the correlation distributions observed in Monte Carlo simulations based on Pythia (Perugia-0 and Perugia 2011 tune), %(though a slight near side hierarchy is present among the three meson results),
